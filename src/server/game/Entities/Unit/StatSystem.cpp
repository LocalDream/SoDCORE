...
bool Player::UpdateStats(Stats stat)
{
    ...
    if (stat == STAT_STRENGTH)
    {
        ...
        if (HasAuraTypeWithMiscvalue(SPELL_AURA_212, stat))
    ...
    else
    {
        ...
        if (HasAuraTypeWithMiscvalue(SPELL_AURA_268, stat))
            UpdateAttackPowerAndDamage(false);
        if (HasAuraTypeWithMiscvalue(SPELL_AURA_212, stat))
            UpdateAttackPowerAndDamage(true);
    }
...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (ranged)
    {
        ...
        {
            case CLASS_HUNTER:
                val2 = 2 * (level + GetStat(STAT_AGILITY) * 2.0f - 20.0f);
                break;
            case CLASS_ROGUE:
    ...
    else
    {
        switch (getClass())
        {
            ...
            case CLASS_DEATH_KNIGHT:
				...
				break;
			case CLASS_ROGUE:
				val2 = (level * 2.0f) + GetStat(STAT_STRENGTH) + GetStat(STAT_AGILITY) * 2.0f - 40.0f;
                break;
            case CLASS_HUNTER:
    ...
    if (ranged)
    {
        ...
        {
            AuraEffectList const& mRAPbyStat = GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_ATTACK_POWER_PCT);
    ...
    {
        AuraEffectList const& mAPbyStat = GetAuraEffectsByType(SPELL_AURA_MOD_RANGED_ATTACK_POWER_PCT);
