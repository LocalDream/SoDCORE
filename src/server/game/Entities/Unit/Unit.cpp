...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    switch (damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_CRIT:
        {
            ...
            // Increase crit damage from SPELL_AURA_169
            mod += GetTotalAuraModifierByMiscMask(SPELL_AURA_169, crTypeMask);
            if (mod != 0)
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
	...
	// Increase from attacker SPELL_AURA_199 auras
    modHitChance += GetTotalAuraModifierByMiscMask(SPELL_AURA_199, schoolMask);

    // Spells with SPELL_ATTR3_IGNORE_HIT_RESULT will ignore target's avoidance effects
    if (!(spell->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT))
    {
        // Chance hit from victim SPELL_AURA_169 auras
        modHitChance += victim->GetTotalAuraModifierByMiscMask(SPELL_AURA_169, schoolMask);
        // Reduce spell hit chance for Area of effect spells from victim SPELL_AURA_160 aura
        if (spell->IsTargetingArea())
            modHitChance -= victim->GetTotalAuraModifier(SPELL_AURA_160);
    ...
    if (!spell->IsPositive())
    {
        ...
        if (bNegativeAura)
        {
            tmp += victim->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_178, int32(spell->Dispel)) * 100;
            tmp += victim->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_178, int32(spell->Dispel)) * 100;
        }
    }
                ...
                case SPELLFAMILY_DRUID:
                {
                    ...
                    if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x10)
                    {
                        if (AuraEffect const* aurEff = aura->GetEffect(EFFECT_1))
                        {
                            ...
                            {
                                int32 mana = CalculatePct(caster->GetCreateMana(), aura->GetSpellInfo()->ManaCostPercentage) * chargesRemoved / 2;
...
bool Unit::HandleHasteAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
	...
	if (!triggerEntry)
    {
        sLog->outError(LOG_FILTER_UNITS,"Unit::HandleHasteAuraProc: Spell %u have not existed triggered spell %u", hasteSpell->Id, triggered_spell_id);
        return false;
    }
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->SpellIconID == 195)
            {
                if (ToPlayer()->HasSpellCooldown(47241)) // Metamorphosis
                    ToPlayer()->RemoveSpellCooldown(47241, -dummySpell->Effects[EFFECT_1].CalcValue()); // Remove X sec
            }
            switch (dummySpell->Id)
            {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            switch (dummySpell->SpellIconID)
            {
                ...
                case 2909: // Cut to the Chase
                {
                    ...
                    if (AuraEffect const* aur = GetAuraEffect(SPELL_AURA_MOD_MELEE_HASTE_3, SPELLFAMILY_ROGUE, 0x40000, 0, 0))
                    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 70811:
                {
                    ...
                    if (procSpell->SpellFamilyFlags[0] & 0x3)
                    {
                        ...
                        {
                            ToPlayer()->RemoveSpellCooldown(16166, -1); // Remove 1 sec
                            return true;
                        }
                    }
                    return false;
                ...
                case 63280: // Glyph of Totem of Wrath
                {
                    ...
                    {
                        int32 bp0 = 0, bp1 = 0;
                        bp0 = CalculatePct(triggerAmount, aurEffA->GetAmount());
                        if (AuraEffect* aurEffB = aurEffA->GetBase()->GetEffect(EFFECT_1))
                            bp1 = CalculatePct(triggerAmount, aurEffB->GetAmount());
