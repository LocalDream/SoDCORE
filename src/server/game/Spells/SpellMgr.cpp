        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            switch (spellInfo->Effects[j].ApplyAuraName)
            {
				...
				case SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS:
                    spellInfo->AttributesCu |= SPELL_ATTR0_CU_NONE2; // Meh i dont think this is a proper name..
                    break; 
                    break;
            }
...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    for (uint32 i = 0; i < mSpellInfoMap.size(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
            ...
            //
			case 98889: // Holy Word: Sanctuary 3yd Heal
	...
	for (uint32 i = 0; i < sTalentTabStore.GetNumRows(); ++i)
    {
        ...
        spellInfo = mSpellInfoMap[talentTab->MasterySpells[0]];
        if (spellInfo)
            spellInfo->AttributesCu |= SPELL_ATTR0_CU_NONE2;

        spellInfo = mSpellInfoMap[talentTab->MasterySpells[1]];
        if (spellInfo)
            spellInfo->AttributesCu |= SPELL_ATTR0_CU_NONE2;
    }
